...
pub fn get(args: &[Arg], session: &Session<Connection>) -> anyhow::Result<()> {
    ...
    let feature_name = feature_arg
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No feature given and none in context. Use `GET <feature>@<identity>` or set a feature context with \"FEATURE use\"."
            )
        })?;
    ...
    let identity_value = identity_arg
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No identity given and none in context. Use `GET <feature>@<identity>` or set an identity context with \"IDENTITY use\"."
            )
        })?;
    ...
    let identity_value = match args.first() {
        None => {
            ...
            ctx.identity
                ...
                .ok_or_else(|| {
                    anyhow::anyhow!(
                        "No identity given and none in context. Use `GETALL @<identity>` or set an identity context with \"IDENTITY use\"."
                    )
                })?
        }
        Some(arg) => arg
