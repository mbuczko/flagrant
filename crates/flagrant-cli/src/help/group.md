...
- `GROUP rejoin [label] [and|and-not]` - stage a connector change for a non-head group
- `GROUP delete [label]` - stage a group deletion by label

*Note: omit `label` on any of the above to pick from an interactive menu instead.*
...
- `GROUP rejoin group-2 and-not` - flip `group-2`'s connector to AND NOT
