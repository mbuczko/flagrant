...
pub async fn create(
    ...
    let variant_id = Variants::create_variant(pool, params!(feature.id, &value), |v| v.get("variant_id"))
        .await
        .map_err(|e| {
    ...
    let weight = Variants::create_variant_weight(pool, params![env.id, variant_id, weight], |v| v.get("weight"))
        .await
        .map_err(|e| {
...
pub async fn fetch(pool: &Pool<Sqlite>, env: &Environment, variant_id: u16) -> anyhow::Result<Variant> {
    Ok(
...
pub async fn list(pool: &Pool<Sqlite>, env: &Environment, feature: &Feature) -> anyhow::Result<Vec<Variant>> {
    Ok(
